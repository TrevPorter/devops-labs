# Git Basics
Document your first week learning Linux commands and structure.

## Key Commands Learned
| Command | Description |
|----------|--------------|
| `git	add .` | Stage any changes |
| `git commit -m "enter comment here"` | adds a descriptive comment to any small changes you made |
| `git push origin main` | Pushes to main branch (GitHub) |

# moving files
| Command | Description |
|----------|--------------|
| `mkdir new-folder-name` | Make a new directory if required |
| `   git mv file-to-move.txt new-folder-name/` | moves a file or files to the folder |
# repeat the commands above to stage and push to GitHub